...
export function useVotingSystem() {
  ...
  const [categoryVotes, setCategoryVotes] = useState<Record<string, Record<string, number>>>({})
    ...
    if (votesParam) {
      try {
        ...
        if (decodedData.theme) {
          setCurrentTheme(decodedData.theme)
          applyTheme(decodedData.theme)
        }
  ...
  const applyTheme = (themeId: string) => {
    if (themeId === "classic" || themeId === "dark") {
      setTheme("dark")
  ...
  const handleCategoryVote = (categoryId: string, countryId: string, points: number) => {
  ...
  const addCustomCategory = () => {
    ...
    setCustomCategories((prev) => [...prev, { id: categoryId, name: newCategoryName }])
    setNewCategoryName("")
